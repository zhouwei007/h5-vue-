...
class AnimationFrame {
  ...
  animationRun (func, scope) {
    ...
    const onAnimationFrameEvent = () => {
      const self = onAnimationFrameEvent
      const $id = self.$id
      if (func() !== false) {
...
class EaseOutEasing extends Easing {
  ...
  getValue () {
    const currentValue = this.startValue + (1 - (1 - (Date.now() - this.startTime) / this.duration) ** (this.exponent)) * this.distance
    if (this.isEnded()) {
...
class BounceEasing extends Easing {
  ...
  getValue () {
    const theta = (Date.now() - this.startTime) / this.acceleration
    return this.startValue - this.startVelocity * theta * (Math.E) ** (-this.springTension * theta)
  }
}
...
class MomentumEasing extends Easing {
  ...
  set friction (value) {
    const theta = Math.log(1 - (value / 10))
    this.$theta = theta
...
class BoundMomentumEasing extends Easing {
  ...
  isEnded () {
    ...
    if (!this.isOutOfBound && Math.abs(momentum.velocity) < this.minVelocity) {
      return true
    }
    if (this.isBouncingBack &&
      (Math.round(this.bounce.getValue()) === (momentum.startVelocity > 0 ? this.minMomentumValue : this.maxMomentumValue))) {
      return true
    }
    return false
  }
  ...
  getValue () {
    ...
    const startVelocity = momentum.startVelocity
    const direction = startVelocity > 0 ? 1 : -1
    const minValue = this.minMomentumValue
    const maxValue = this.maxMomentumValue
    const boundedValue = (direction === 1) ? minValue : maxValue
    ...
    if (!this.isOutOfBound) {
      ...
      if (maxValue === undefined && value > minValue) {
        return value
      }
